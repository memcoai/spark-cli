...
describe('parseTags', () => {
  ...
  describe('TYPE:NAME:VERSION format', () => {
    it('parses tag with valid semver', () => {
      assert.deepStrictEqual(parseTags('language_version:node:1.2.3'), [
        'language_version:node:1.2.3',
      ]);
    });

    it('strips v prefix from version', () => {
      assert.deepStrictEqual(parseTags('language_version:node:v1.2.3'), [
        'language_version:node:1.2.3',
      ]);
    });

    it('allows pre-release suffix in patch', () => {
      assert.deepStrictEqual(parseTags('library_version:react:18.2.0-beta'), [
        'library_version:react:18.2.0-beta',
      ]);
    });

    it('allows pre-release identifiers with dots', () => {
      assert.deepStrictEqual(parseTags('framework_version:next:14.0.0-rc.1'), [
        'framework_version:next:14.0.0-rc.1',
      ]);
    });

    it('accepts major-only version', () => {
      assert.deepStrictEqual(parseTags('language_version:node:22'), ['language_version:node:22']);
    });

    it('accepts major-only version with v prefix', () => {
      assert.deepStrictEqual(parseTags('language_version:node:v22'), ['language_version:node:22']);
    });

    it('accepts major.minor version', () => {
      assert.deepStrictEqual(parseTags('language_version:node:22.11'), [
        'language_version:node:22.11',
      ]);
    });

    it('accepts major.minor version with v prefix', () => {
      assert.deepStrictEqual(parseTags('language_version:node:v22.11'), [
        'language_version:node:22.11',
      ]);
    });

    it('mixes TYPE:NAME and TYPE:NAME:VERSION tags', () => {
      assert.deepStrictEqual(parseTags('task_type:bug_fix,language_version:node:20.0.0'), [
        'task_type:bug_fix',
        'language_version:node:20.0.0',
      ]);
    });
  });
  ...
  describe('validation errors', () => {
    ...
    it('throws on non-numeric version', () => {
      assert.throws(() => parseTags('language_version:node:latest'), {
        message: /Invalid version "latest".*expected a numeric version/,
      });
    });

    it('throws on non-integer major', () => {
      assert.throws(() => parseTags('language_version:node:abc.0.0'), {
        message: /Invalid version "abc.0.0"/,
      });
    });

    it('throws on non-integer minor', () => {
      assert.throws(() => parseTags('language_version:node:1.abc.0'), {
